function genRangeNumber(start,finish){
  var num=(Math.round(((finish-start)*Math.random())*100)/100+start)
	return num;
}
...
function roundTwoCiphers(num){
  return Math.round(num*100)/100
}

function roundFourCiphers(num){
  return Math.round(num*10000)/10000
}


function strCatSum(str2,num){
  num = roundTwoCiphers(num);
	str2 = str2 + " + " + num;
	return str2;
}
...
function GenQuestion(startRange,finishRange,Operations,time,validOperations){
    ...
    for (var i = 0; i < arr.length; i++) {
      arr[i]==roundTwoCiphers(arr[i]);
    };
    for (var i = 0; i < ope.length; i++) {
      ope[i]==roundTwoCiphers(ope[i]);
    };
  ...
  for (var i = 0; i < optionArray.length; i++) {
    optionArray[i]=roundFourCiphers(optionArray[i]);
  };
